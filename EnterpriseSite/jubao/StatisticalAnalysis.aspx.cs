...
public partial class jubao_StatisticalAnalysis : System.Web.UI.Page
{
    ...
    private void BindQuestionList(string classType, string className)
    {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    argLoad = true;
                    array[i + 1] = dList.Rows.Count.ToString();
                    if (IsLoad)
                    {
                        array[0] = className;
                    }
    ...
    private void AddCell(HtmlTableRow tr, string text, string href)
    {
        ...
        tc.Width = "5%";
        tc.InnerHtml = text;
        if (href.Length > 0) tc.Attributes["onclick"] = "javascript:window.location.href='StatisticalAnalysis.aspx?pid=" + href + "'";
        tc.Attributes["class"] = "ListCellRow2";
        tr.Cells.Add(tc);
    }
    protected void chartLink_Click(object sender, EventArgs e)
    {
        Response.Redirect("StatisticalAnalysisChart.aspx");
    }
}